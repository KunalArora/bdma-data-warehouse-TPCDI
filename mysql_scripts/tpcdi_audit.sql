...
select 'DimAccount IsCurrent', NULL, case when
	(select count(*) from DimAccount) =
	(select count(*) from DimAccount where EndDate = '9999-12-31' and IsCurrent = true) +
	(select count(*) from DimAccount where EndDate < '9999-12-31' and IsCurrent = false)
...
select 'DimCustomer EndDate', NULL, case when
	...
	(select count(*) from DimCustomer where EndDate = '9999-01-01 12:00:00')
then 'OK' else 'Dates not aligned' end, 'EndDate of one record matches EffectiveDate of another, or the end of time'
...
select 'DimCustomer IsCurrent', NULL, case when 
	(select count(*) from DimCustomer) =
	(select count(*) from DimCustomer where EndDate = '9999-01-01 12:00:00' and IsCurrent = true) +
	(select count(*) from DimCustomer where EndDate < '9999-01-01 12:00:00' and IsCurrent = false)
then 'OK' else 'Not current' end, 'IsCurrent is 1 if EndDate is the end of time, else Iscurrent is 0'
...
-- 	select distinct BatchID, case when
-- 		(select messageData from DImessages where MessageType = 'Validation' and BatchID = a.BatchID and 'DimCustomer' = MessageSource and 'Inactive customers' = MessageText):: INT8 = 
